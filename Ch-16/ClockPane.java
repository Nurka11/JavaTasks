...
public class ClockPane extends Application{
	...
	public void start(Stage gui){
		...
		clock.paintClock();
		TextField ho=new TextField("Hour");
		TextField mi=new TextField("Minute");
		TextField se=new TextField("Second");
		ho.setOnAction(e->{
			hour=Integer.parseInt(ho.getText());
			clock.paintClock();
		});
		mi.setOnAction(e->{
			minute=Integer.parseInt(mi.getText());
			clock.paintClock();
		});
		se.setOnAction(e->{
			second=Integer.parseInt(se.getText());
			clock.paintClock();
		});
		HBox hours=new HBox();
		hours.getChildren().addAll(ho,mi,se);
		pane.getChildren().addAll(clock,hours);
